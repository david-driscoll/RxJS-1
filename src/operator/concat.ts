...
/* tslint:enable:max-line-length */
export function concatStatic<T, R>(...observables: Array<ObservableInput<any> | Scheduler>): Observable<T | R> {
  ...
  return new ArrayObservable(observables, scheduler).lift<ObservableInput<T>, T | R>(new MergeAllOperator<T | R>(1));
}
