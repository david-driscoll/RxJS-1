...
describe('Subscription', () => {
  it('should not leak', (done) => {
    ...
    const source1 = new Observable<any>((observer: Rx.Observer<any>) => {
      return () => {
        tearDowns.push(1);
      };
    });

    const source2 = new Observable<any>((observer: Rx.Observer<any>) => {
      return () => {
        tearDowns.push(2);
    ...
    const source3 = new Observable<any>((observer: Rx.Observer<any>) => {
      return () => {
        tearDowns.push(3);
      };
    });
