...
describe('Observable.fromEvent', () => {
  asDiagram('fromEvent(element, \'click\')')
    ('should create an observable of click on the element', () => {
      const target = {
        addEventListener: (eventType: string, listener: (x: any) => void) => {
        ...
        removeEventListener: (eventType: string, listener: Function): void => void 0,
        dispatchEvent: (): void => void 0,
      };
      const e1 = Observable.fromEvent(<any>target, 'click');
      const expected = '-----x-x---';
