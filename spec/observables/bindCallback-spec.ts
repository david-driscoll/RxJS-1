...
describe('Observable.bindCallback', () => {
  describe('when not scheduled', () => {
    ...
    it('should set callback function context to context of returned function', () => {
      function callback(this: { datum: number }, cb: (value: number) => void) {
        cb(this.datum);
      }
    ...
    it('should emit one value chosen by a selector', () => {
      ...
      const boundCallback = Observable.bindCallback(callback, (datum) => datum);
      const results: (number | string)[] = [];
    ...
    it('should emit an error when the selector throws', () => {
      ...
      const boundCallback = Observable.bindCallback(callback, (err) => { throw expected; });
  ...
  it('should pass multiple inner arguments as an array', () => {
    function callback(datum: number, cb: (...args: number[]) => void) {
      cb(datum, 1, 2, 3);
    }
    const boundCallback = Observable.bindCallback<number, number[]>(callback, null, rxTestScheduler);
    const results: (number[] | string)[] = [];
  ...
  it('should pass multiple inner arguments to the selector if there is one', () => {
    function callback(datum: number, cb: (value: number, ...args: number[]) => void) {
      cb(datum, 1, 2, 3);
    }
