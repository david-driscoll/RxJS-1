...
describe('Observable.webSocket', () => {
  ...
  describe('multiplex', () => {
    ...
    it('should keep the same socket for multiple multiplex subscriptions', () => {
      const socketSubject = Rx.Observable.webSocket<{ id: string; complete: boolean; }>({url: 'ws://mysocket'});
      const results: string[] = [];
    ...
    it('should not close the socket until all subscriptions complete', () => {
      const socketSubject = Rx.Observable.webSocket<{ id: string; complete: boolean; }>(<any>{url: 'ws://mysocket'});
      const results: string[] = [];
      const socketMessages = [
        {id: 'A'},
        {id: 'B'},
